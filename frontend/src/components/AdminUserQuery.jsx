...
const AdminQueryManagement = ({ activeTab }) => {
  ...
  useEffect(() => {
    // Inline filter logic to avoid missing dependency warning
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter(q => 
        ...
        q.User?.fullName?.toLowerCase().includes(lower) ||
        q.User?.email?.toLowerCase().includes(lower)
      );
    }
  ...
  const fetchAllQueries = async () => {
    ...
    try {
      ...
      const response = await fetch(apiUrl('/admin/user-queries'), {
        method: 'GET',
        headers: {
  ...
  // (filterQueries helper removed; logic inlined in effect)
  ...
  const handleUpdateQueryStatus = async (e) => {
    ...
    if (!responseText.trim() && (selectedQuery.status === 'pending' || selectedQuery.status === 'in-progress')) {
      alert('Please provide a response before marking as resolved or closed');
      return;
    }
    ...
    try {
      ...
      const response = await fetch(apiUrl(`/admin/user-queries/${selectedQuery.id}`), {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      alert(err.message || 'Failed to update query. Please try again.');
    } finally {
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-col md:flex-row md:space-x-4 space-y-3 md:space-y-0">
          ...
          <div className="relative flex-grow">
            ...
            <input
              type="text"
              placeholder="Search by subject, description, or user info..."
              value={searchTerm}
      ...
      <div className="space-y-4">
        ...
        <div className="bg-white border rounded-lg overflow-hidden">
          ...
          <div className="p-4 bg-gray-50 border-b">
            <h3 className="text-sm font-medium mb-2" style={{ color: styles.primaryBlue }}>User Information:</h3>
            <div className="flex flex-wrap gap-4">
              <div>
                <span className="text-xs text-gray-500">Full Name</span>
                <p className="font-medium" style={{ color: styles.textDark }}>
                  {selectedQuery.User?.fullName || 'Not provided'}
                </p>
              </div>
              <div>
                <span className="text-xs text-gray-500">Email</span>
                <p className="font-medium" style={{ color: styles.textDark }}>
                  {selectedQuery.User?.email || 'Not provided'}
                </p>
              </div>
              <div>
                <span className="text-xs text-gray-500">User ID</span>
                <p className="font-medium" style={{ color: styles.textDark }}>
                  {selectedQuery.userId || 'Not available'}
                </p>
              </div>
            </div>
          </div>
