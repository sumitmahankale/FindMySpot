...
import { Users, Map, Layout, ChevronRight, Bell, Search, LogOut, Settings } from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  return (
    <div className={`flex h-screen bg-gray-100 overflow-hidden ${pageLoaded ? 'opacity-100' : 'opacity-0'}`} 
         style={{ transition: 'opacity 0.5s ease-in-out' }}>
      {/* Side Navigation */}
      <div 
        className={`${isCollapsed ? 'w-20' : 'w-64'} bg-blue-900 text-white shadow-xl transition-all duration-300 ease-in-out flex flex-col`}
      >
          ...
          <button
            className={`flex items-center w-full px-6 py-4 text-left transition-all duration-200 ${
              activeTab === 'requests' 
                ...
                : 'hover:bg-blue-800 hover:border-l-4 hover:border-blue-400'
            }`}
        ...
        <div className={`px-6 py-4 border-t border-blue-800 w-full ${isCollapsed ? 'flex justify-center' : ''}`}>
          {isCollapsed ? (
            ...
            <div className="flex items-center justify-between">
              ...
              <button className="text-blue-300 hover:text-white transition-colors" onClick={handlelogout}>
